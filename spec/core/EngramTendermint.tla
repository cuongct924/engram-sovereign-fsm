...
IsValidProposal(prop) ==
    ...
    IN
        ...
        \* Healthy cross-check (x/sovereignty/proposal.go:299-303): fsm_state
        \* alone can't distinguish genuinely healthy from
        \* warning-but-hysteresis-absorbed, since both hold the FSM at its
        \* current state -- but unhealthy_streak's next value (and hence every
        \* future transition) depends on which one actually happened. Without
        \* this, honest validators could silently diverge on unhealthy_streak
        \* while still agreeing on fsm_state.
