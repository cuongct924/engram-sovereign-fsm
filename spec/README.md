...
* **Trace:** honest node `n2` (not Byzantine) senses a BTC SPV failure, correctly computes `fsm_state = "SOVEREIGN"` for its proposal, and bundles `value = "TX_WITHDRAWAL"` in that same proposal. With the conjunct gone, `IsValidProposal` accepts it.
...
* **Trace:** the FSM reaches `RECOVERING` with `safe_blocks = 0`. The instant `reanchoring_proof_valid` flips to `TRUE` (BTC anchor catches up to the submission height), the ablated spec transitions straight to `ANCHORED` — zero blocks of the required `HYSTERESIS_WAIT = 2` wait.
* **Why it matters:** the hysteresis counter isn't cosmetic — without it, a single favorable instant (proof valid) is enough to re-anchor mid-recovery, even if the network's health is still noisy. The checked invariant is a ghost-variable equivalent of the real `HysteresisSafety`: Apalache's `--inv` only checks current-state predicates, so the driver latches a boolean the instant an illegal `RECOVERING -> ANCHORED` move fires. A control run of the same driver against the non-ablated `EngramFSM.tla` reached depth 8 with no violation, confirming this is a real consequence of the ablation and not an artifact of the driver's `MC_FSMNext`.
...
* **Trace:** `active_peers` becomes entirely Sybil-controlled (`anchor_peers` unreachable — a full eclipse), pushing the FSM to `SOVEREIGN`. With the P2P conjunct gone, `IsHealthyCondition` no longer looks at peer quality, so the very next step transitions straight to `RECOVERING` while the node is still fully eclipsed.
* **Why it matters:** `IsHealthyCondition` in the real spec is a plain conjunction (`~IsBTCGapSovereign /\ ~IsBTCGapSuspicious /\ IsDAHealthy /\ IsP2PQualityHealthy`), so this check is a structural guarantee, not just an empirically-unreached case: no branch of `CalculateNextFSMState` can produce `RECOVERING` while `~IsP2PQualityHealthy` holds. The ablation is what makes it reachable — an eclipsed node can look "recovered" using only BTC/DA signals, even with zero honest peer connectivity.
...
Install the TLA+ extension → open the `.tla` file → Command Palette → `TLA+: Check model with TLC` → pick the matching `.cfg`.
