...
Live run against the real 4-node testnet. Confirms `FilterPeerByAddr` (`x/sovereignty/
...
Run twice (the first run hit a real teardown bug that destroyed the whole cluster --
...
Confirmed via `docker inspect`/`/net_info`: for a container attached to MULTIPLE Docker
networks with no explicit `gw_priority` override, Docker's default outbound route
consistently prefers whichever network was declared SECOND, not the container's own
dedicated subnet:
...
(if undocumented) Docker Compose default-gateway behavior. A `gw_priority` fix (confirmed to
set `GwPriority` correctly via `docker inspect`) was attempted on one test container but did
not change the observed connection subnet -- left as a genuine, unresolved limitation of this
...
demonstrated by A1/A2 both holding at exactly 8).

**Practical implication for E4/E8's A2 row**: this Docker topology cannot cleanly demonstrate
"genuine multi-subnet Sybil evades a single-subnet cap" as originally scoped -- what WAS
demonstrated live is that the filter's cap holds under both a same-subnet swarm (A1) and a
larger swarm that (unintentionally, due to this Docker quirk) also landed on one subnet (A2)
-- a real, valid safety result, just not the specific subnet-diversity-evasion scenario A2
set out to test. A full fix would need either resolving the gw_priority behavior, or moving
off Docker's default bridge networking (e.g. macvlan) for genuinely independent source IPs
per attacker.

## Bugs found and fixed during this run

1. **`docker compose ... down` (no service names) tears down the ENTIRE project**, not just
   the profile-gated attacker services -- `live_sybil_attack.py`'s `swarm_down()` used this
   and destroyed the running real 4-node cluster mid-experiment (all containers removed,
   including bitcoin/celestia). Fixed to use `docker compose stop <services>` + `rm -f
   <services>` with explicit service name lists, matching
   `scripts/framework/injector.py`'s `cleanup_profile` convention. The same bug also existed
   in `live_combined_attack.py` and `live_double_signing_test.py`, fixed identically in both.
2. **`docker/attacker-peer-swarm.yml`'s `persistent_peers` was missing the target's node
   ...
   entrypoint script resolve the real target ID via `curl .../status | jq -r
...
4. **Self-inflicted bitcoind desync (not a repo bug)**: burst-mining 101 blocks to fund a
   test wallet while the engramd containers were already live pushed `h_btc_current` far
   outside `vigilante.VerifyReceipt`'s tolerance window, stalling consensus permanently
   (every proposal rejected at check #3) -- the exact failure mode documented in
   `scripts/bitcoin_miner_loop.sh`'s own comment. Fixed by always funding + maturing the
   mining wallet before starting any engramd container, never mid-run. A related mismatch
   also contributed: the miner loop defaults to wallet name `engramwallet`, but the wallet
   created ad hoc here was named `minerwallet` -- the mismatch silently killed the miner
   loop (`set -e` + RPC error), compounding the stall.
