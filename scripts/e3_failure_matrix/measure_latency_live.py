...
the live CSVs do NOT carry btc_gap/da_healthy/p2p_healthy columns at all --
x/sovereignty/preblock.go's NewPreBlocker deliberately never writes live
PeripheralMetrics into committed state (see this repo's own documented
safety fix: doing so once caused a real AppHash-divergence consensus
failure across the 4-node testnet, reverted for exactly this reason). The
in-process Harness has no such constraint since it isn't ABCI-committed
state at all. So this table reports what IS honestly observable live
(fsm_state, height, withdrawal-lock status computed from fsm_state alone),
and explicitly omits the BTC/DA/P2P condition columns rather than
fabricating them from unavailable data.
