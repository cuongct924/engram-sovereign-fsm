...
def load_rows():
    ...
    for r in rows:
        ...
        for k in ("da_healthy", "p2p_healthy", "withdraw_locked", "proof_submitted", "block_committed"):
            r[k] = r[k].lower() == "true"
    return rows
...
def plot_figure2(rows):
    ...
    axes[0].step(heights, [STATE_Y[r["state"]] for r in rows], where="post", linewidth=2, color="black")
    ...
    axes[2].step(heights, [0 if r["da_healthy"] else 1 for r in rows], where="post", color="C1")
    axes[2].set_yticks([0, 1])
    axes[2].set_yticklabels(["healthy", "outage"], fontsize=8)
    axes[2].set_title("DA Gap (availability)")

    axes[3].step(heights, [1 if r["p2p_healthy"] else 0 for r in rows], where="post", color="C2")
    axes[3].set_yticks([0, 1])
    axes[3].set_yticklabels(["unhealthy", "healthy"], fontsize=8)
    axes[3].set_title("P2P Health Score")

    axes[4].step(heights, [1 if r["block_committed"] else 0 for r in rows], where="post", color="C3")
    ...
    axes[5].step(heights, [1 if r["proof_submitted"] else 0 for r in rows], where="post", color="C4", label="proof_submitted")
    ...
    fig.suptitle("Figure 2 -- FSM Timeline Under a Combined-Failure Trace (real tests/e2e data)")
    fig.tight_layout()
...
def main():
    if not os.path.exists(CSV_IN):
        print(f"{CSV_IN} not found -- run 'go test ./tests/e2e/... -run TestE9_TraceDrivenCombinedFailure' first.", file=sys.stderr)
        sys.exit(1)
    rows = load_rows()
    plot_figure2(rows)
