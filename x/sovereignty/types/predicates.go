...
// total anchor-peer loss (complete eclipse), or a SUSPICIOUS gray-failure timeout.
...
func IsHealthyCondition(m *PeripheralMetrics, p Params) bool {
	return !IsBTCGapSovereign(m, p) && !IsBTCGapSuspicious(m, p) && IsDAHealthy(m, p) && IsP2PQualityHealthy(m, p)
}
